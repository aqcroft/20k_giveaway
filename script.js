const CONFIG = {
  ...
  // TODO: confirm this exact slug in TidyCal before going live
  refresherBookingUrl: "https://tidycal.com/aqcroft/uwcrefresher",
...
if (profileBubble && profileBubbleBtn && profileBubbleMenu) {
  ...
  // Show the bubble only after the hero has scrolled away, so it doesn't
  // sit on top of the partner card at the top of the page.
  const onScroll = () => {
    if (window.scrollY > 120) {
      profileBubble.classList.add("visible");
    } else {
      profileBubble.classList.remove("visible");
      setBubbleOpen(false);
    }
  };
